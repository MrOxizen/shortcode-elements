...
function oxi_mailChimp_style_1_shortcode($styledata = false, $listdata = false, $user = 'user') {
    OxiAddonsAjaxRequest();
    add_action('wp_print_scripts', 'OxiAddonsAjaxRequest');
    $oxiid = $styledata['id'];
    $stylefiles = explode('||#||', $styledata['css']);
    $styledata = explode('|', $stylefiles[0]);
    $leftbutton = $rightbutton = $jquery = $placeholder = $icon = $error_msg = $success_msg = $loading_msg = $button_text = '';
    if ($styledata[5] == 'left') {
        $leftbutton .= '<div class="oxi-addons-mailchimp-button-section">
                            <button type="submit" class="oxi-addons-mailchimp-button oximailChimpEqual-'.$oxiid.'"  ' . OxiAddonsAnimation($styledata, 199) . '>'
    ...
    $jquery .= "
        jQuery('#oxi-addons-mailchimp-full-content-$oxiid').submit(function (e) {
            ...
            jQuery(this)[0].reset();
            OxiAddonsAjaxDataProcess( $oxiid, 'oxi_mailChimp_style_1_request', useremail, 'mailChimp||style-1', '', function (data) {
                if (data === 'success') {
...
function oxi_mailChimp_style_1_request($styleid, $file) {
