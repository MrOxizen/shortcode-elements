...
if (!empty($_POST['data-submit']) && $_POST['data-submit'] == 'Save') {
    if (!wp_verify_nonce($nonce, 'oxi-addons-nonce')) {
        ...
        $data = 'oxi-addons-link-opening |' . sanitize_text_field($_POST['oxi-addons-link-opening']) . '|'
                ...
                . ' oxi-addons-border-hover-color |' . sanitize_text_field($_POST['oxi-addons-border-hover-color']) . '|'
                . '||#||  ||#||'
                        ...
                        <div class="oxi-addons-tabs-content-tabs active">
                            ...
                            <div class="oxi-addons-col-6">
                                ...
                                <div class="oxi-addons-content-div">
                                    ...
                                    <div class="oxi-addons-content-div-body">
                                        <?php
                                        echo oxi_addons_adm_help_number_dtm('oxi-addons-border-size', $styledata[43], $styledata[44], $styledata[45], 1, 'Border Size', 'Your Border Size', '', '.oxi-links-effects-' . $oxiid . '::before', 'height');
                                        echo oxi_addons_adm_help_MiniColor('oxi-addons-border-color', $styledata[11], '', 'Border Color', 'Select Your Border Color', '', '');
                                        echo oxi_addons_adm_help_MiniColor('oxi-addons-border-hover-color', $styledata[47], '', 'Border Hover Color', 'Select Your Border Hover Color', '', '');
                                        ?>
                                    </div>
                                </div>   
                            </div>
...
<script type="text/javascript">
    jQuery("#oxi-addons-border-size").on("change", function () {
        var value1 = jQuery(this).val();
        jQuery("<style type='text/css'> #oxi-addons-preview-data .oxi-links-effects-<?php echo $oxiid; ?>::after{width:" + value1 + "px; } </style>").appendTo("#oxi-addons-preview-data");
    ...
    jQuery("#oxi-addons-border-color").on("change", function () {
        var value1 = jQuery(this).val();
        jQuery("<style type='text/css'> #oxi-addons-preview-data  .oxi-links-effects-<?php echo $oxiid; ?>::before{background: " + value1 + "; } </style>").appendTo("#oxi-addons-preview-data");
        jQuery("<style type='text/css'> #oxi-addons-preview-data  .oxi-links-effects-<?php echo $oxiid; ?>::after{background:" + value1 + "; } </style>").appendTo("#oxi-addons-preview-data");
    });
    jQuery("#oxi-addons-border-hover-color").on("change", function () {
        var value1 = jQuery(this).val();
        jQuery("<style type='text/css'> #oxi-addons-preview-data  .oxi-links-effects-<?php echo $oxiid; ?>:hover::before, .oxi-links-effects-<?php echo $oxiid; ?>:focus::before{background: " + value1 + "; } </style>").appendTo("#oxi-addons-preview-data");
        jQuery("<style type='text/css'> #oxi-addons-preview-data  .oxi-links-effects-<?php echo $oxiid; ?>:hover::after, .oxi-links-effects-<?php echo $oxiid; ?>:focus::after{background:" + value1 + "; } </style>").appendTo("#oxi-addons-preview-data");
    });
</script>